...
def training(neuralnet, dataset, epochs, batch_size, normalize=True):
    ...
    for epoch in range(epochs):
        ...
        step_dict = neuralnet.step(x=x_tr, z=z_tr, training=False)
        ...
        if(neuralnet.zdim == 2):
            ...
            step_dict = neuralnet.step(x=x_tr, z=z_latents, training=False)
            x_fake = step_dict['x_fake']
            z_tr = random_noise(x_tr.shape[0], neuralnet.zdim)
        ...
        while(True):
            ...
            z_tr = random_noise(x_tr.shape[0], neuralnet.zdim)
            step_dict = neuralnet.step(x=x_tr, z=z_tr, iteration=iteration, training=True)
...
def test(neuralnet, dataset, batch_size):
    ...
    for i in range(10):
        ...
        step_dict = neuralnet.step(x=x_te, z=z_te, training=False)
        x_fake = step_dict['x_fake']
